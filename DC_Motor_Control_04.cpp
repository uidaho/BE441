...
int Err; // Variable to store error for PI control
int u = SetRPM; // Control signal initialized to desired RPM
...
void loop() {
   ...
   RPM = Count * 0.5102; // Convert count to RPM (based on encoder and gear ratio)
   Count = 0; // Reset counter
   
   // Print the current RPM setting, calculated RPM, and control signal to the serial monitor
   Serial.print(SetRPM);
   Serial.print(",");
   Serial.print(RPM);
   Serial.print(",");
...
void PIDControl() {
   // Calculate the error between the desired RPM and the actual RPM
   Err = SetRPM - RPM;
   
   // Update the control signal by accumulating (= integrating) the error
   u += Err;
   
   // Set the motor speed based on the updated control signal
   runMotor(u);
}
